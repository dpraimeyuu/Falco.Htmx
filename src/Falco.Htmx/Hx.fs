...
module Trigger =
    let event (name: string, filters: string option, modifiers: EventModifier list) = Event(name, filters, modifiers)
...
let select (option: TargetOption) =
    Attr.create "hx-select" (TargetOption.AsString option)

/// Select content to swap in from a response, out of band (somewhere other than the target)
let selectOob (option: TargetOption) =
    Attr.create "hx-select-oob" (TargetOption.AsString option)

/// Controls how content is swapped in (outerHTML, beforeEnd, afterend, ...)
let swap (option: SwapOption) =
    Attr.create "hx-swap" (SwapOption.AsString option)
...
let target (option: TargetOption) =
    Attr.create "hx-target" (TargetOption.AsString option)
...
let indicator (option: TargetOption) =
    Attr.create "hx-indicator" (TargetOption.AsString option)
